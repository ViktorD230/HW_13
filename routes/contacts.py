...
async def search_contacts(first_name: str = None, last_name: str = None, email: EmailStr = None, db: Session = Depends(get_db),
                          current_user: User = Depends(auth_service.get_current_user)):
    contacts = await repository_contacts.search_contacts(first_name, last_name, email, current_user, db)
    if len(contacts) == 0:
